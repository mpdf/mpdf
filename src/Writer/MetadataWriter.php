			...
			if (isset($this->mpdf->PageLinks[$n]) || isset($this->mpdf->PageAnnots[$n]) || count($this->form->forms) > 0) {
				...
				if (isset($this->mpdf->PageLinks[$n])) {
					...
					foreach ($this->mpdf->PageLinks[$n] as $key => $pl) {
						...
						// $this->writer->write(' /Contents ' . $this->writer->utf16BigEndianTextString($pl[4]), false);
						...
						// this->writer->write(' /BS <</W 1', false);	// Width on points; 0 = no line
						// $this->writer->write(' /S /D', false);		// style - [S]olid, [D]ashed, [B]eveled, [I]nset, [U]nderline
						// $this->writer->write(' /D [3 2]', false);		// Dash array - if dashed
						// $this->writer->write(' >>', false);
						// $this->writer->write(' /C [1 0 0]', false);	// Color RGB
						...
						if (strpos($pl[4], '@') === 0) {
							...
							$this->writer->write(sprintf(' /Dest [%d 0 R /XYZ 0 %.3F null]>>', 1 + 2 * $p, $htarg), false);

						} elseif (is_string($pl[4])) {
							if (isset($pl['importedLink'])) {
								$this->writer->write('/A <</S /URI /URI ' . $this->writer->string($pl[4]) . '>>', false);
