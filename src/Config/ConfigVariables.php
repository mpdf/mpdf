...
class ConfigVariables
{
	...
	// AUTOMATIC FONphp getT SELECTION
	// Based on script and/or language
	// mPDF 6.0 (similar to previously using function SetAutoFont() )
	protected $autoScriptToLang = false;
	protected $baseScript = Ucdn::SCRIPT_LATIN;
	protected $autoVietnamese = true;
	protected $autoArabic = true;
	...
	protected $incrementFPR1 = 10; // i.e. will alter by 1/[10]th of width and try again until within closer limits
	protected $incrementFPR2 = 20;
	protected $incrementFPR3 = 30;
	protected $incrementFPR4 = 50; // i.e. will alter by 1/[50]th of width and try again when it nearly fits
	...
	protected $title2annots = false; // Automatically convert title="" properties in tags, to annotations
	protected $annotSize = 0.5; // default mm for Adobe annotations - nominal
	protected $annotMargin = null; // default position for Annotations
	protected $annotOpacity = 0.5; // default opacity for Annotations
...
protected $h2bookmarks = array('H1'=>0, 'H2'=>1, 'H3'=>2);
	*/
	protected $h2bookmarks = [];
	...
	/*
		...
		H1 - H6 must be uppercase
protected $h2toc = array('H1'=>0, 'H2'=>1, 'H3'=>2);
	*/
	protected $h2toc = [];
	...
	/* Specifies whether to repeat the main entry for each subEntry (true suppresses this)
		...
		Mammal, elephant
	*/
	protected $indexUseSubentries = true;
	...
	protected $defaultheaderfontsize = 8; // pt
	protected $defaultheaderfontstyle = 'BI'; // '', or 'B' or 'I' or 'BI'
	protected $defaultheaderline = 1; // 1 or 0 - line under the header
	protected $defaultfooterfontsize = 8; // pt
	protected $defaultfooterfontstyle = 'BI'; // '', or 'B' or 'I' or 'BI'
	protected $defaultfooterline = 1; // 1 or 0 - line over the footer
	...
	protected $keep_table_proportions = true;
	...
	protected $interpolateImages = false;
	...
	protected $whitelistStreamWrappers = ['http', 'https', 'file'];
	...
	// COLUMNS
	protected $keepColumns = false; // Set to go to the second column only when the first is full of text etc.
	protected $max_colH_correction = 1.15; // Maximum ratio to adjust column height when justifying - too large a value can give ugly results
	protected $ColGap = 5;
	...
	// Hyphenation
	protected $SHYlanguages = ['en', 'de', 'es', 'fi', 'fr', 'it', 'nl', 'pl', 'ru', 'sv']; // existing defined patterns
	protected $SHYlang = "en"; // 'en','de','es','fi','fr','it','nl','pl','ru','sv'
	protected $SHYleftmin = 2;
	protected $SHYrightmin = 2;
	protected $SHYcharmin = 2;
	protected $SHYcharmax = 10;

	// ACTIVE FORMS
	protected $useActiveForms = false;

	// WATERMARKS
	protected $watermarkImgBehind = false;
	protected $showWatermarkText = 0;
	protected $showWatermarkImage = 0;
	protected $watermarkText = '';
	protected $watermarkAngle = 45;
	...
	// BORDERS
	protected $autoPadding = false; // Automatically increases padding in block elements when border-radius set - if required
	...
	protected $svgClasses = false;

	// Default values if no style sheet offered	(cf. http://www.w3.org/TR/CSS21/sample.html)
	protected $defaultCSS = null;
	protected $defaultCssFile = __DIR__ . '/../../data/mpdf.css';

	protected $customProperties = [];

	protected $languageToFont = null;
	protected $scriptToLanguage = null;

	//////////////////////////////////////////////////
	// VALUES ONLY LIKELY TO BE CHANGED BY DEVELOPERS
	//////////////////////////////////////////////////

	protected $pdf_version = '1.4';
	...
	protected $allowAnnotationFiles = false;

	protected $hyphenationDictionaryFile = __DIR__ . '/../../data/patterns/dictionary.txt';

	protected $default_lineheight_correction = 1.2; // Value 1 sets lineheight=fontsize height,
	// Value used if line-height not set by CSS (usually is)

	protected $fontsizes = ['XX-SMALL' => 0.7, 'X-SMALL' => 0.77, 'SMALL' => 0.86, 'MEDIUM' => 1, 'LARGE' => 1.2, 'X-LARGE' => 1.5, 'XX-LARGE' => 2];

	// CHARACTER PATTERN MATCHES TO DETECT LANGUAGES
	// pattern used to detect RTL characters -> force RTL
	protected $pregRTLchars = "\x{0590}-\x{06FF}\x{0700}-\x{085F}\x{FB00}-\x{FDFD}\x{FE70}-\x{FEFF}"; // 085F to include Mandaic
	// Chars which distinguish CJK but not between different
	protected $pregCJKchars = "\x{1100}-\x{11FF}\x{2E80}-\x{A4CF}\x{A800}-\x{D7AF}\x{F900}-\x{FAFF}\x{FE30}-\x{FE6F}\x{FF00}-\x{FFEF}\x{20000}-\x{2FA1F}";
	...
	protected $CJKoverflow = "\.,\x{ff61}\x{ff64}\x{3001}\x{3002}\x{fe50}-\x{fe52}\x{ff0c}\x{ff0e}";
	...
	protected $pregCURSchars = "\x{0590}-\x{083E}\x{0900}-\x{0DFF}\x{FB00}-\x{FDFD}\x{FE70}-\x{FEFF}";

	protected $allowedCSStags = 'DIV|P|H1|H2|H3|H4|H5|H6|FORM|IMG|A|BODY|TABLE|HR|THEAD|TFOOT|TBODY|TH|TR|TD|UL|OL|LI|PRE|BLOCKQUOTE|ADDRESS|DL|DT|DD'
	...
	protected $outerblocktags = ['DIV', 'FORM', 'CENTER', 'DL', 'FIELDSET', 'ARTICLE', 'ASIDE', 'FIGURE', 'FIGCAPTION', 'FOOTER', 'HEADER', 'HGROUP', 'MAIN', 'NAV', 'SECTION', 'DETAILS', 'SUMMARY', 'UL', 'OL', 'LI'];
	protected $innerblocktags = ['P', 'BLOCKQUOTE', 'ADDRESS', 'PRE', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'DT', 'DD', 'CAPTION'];

	// cURL options
	protected $curlFollowLocation = false;
	protected $curlAllowUnsafeSslRequests = false;
	protected $curlCaCertificate = '';
	protected $curlTimeout = 5;
	protected $curlProxy = null;
	protected $curlProxyAuth = null;
	protected $curlUserAgent = 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:13.0) Gecko/20100101 Firefox/13.0.1';

	protected $exposeVersion = true;

	public function __construct()
	{
		$this->defaultCSS = DefaultCss::$definition;

		$this->languageToFont = new LanguageToFont();
		$this->scriptToLanguage = new ScriptToLanguage();
	}
